  ...
  return (
    <div className="pt-32 pb-20 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
        <h1 className="font-serif text-5xl md:text-6xl text-shadow dark:text-smoke mb-6">Collections</h1>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`text-xs uppercase tracking-widest transition-all duration-300 ${
                activeCategory === cat 
                  ? 'text-shadow dark:text-smoke border-b border-gold pb-1' 
        ...
        {filteredProducts.map((product) => (
          ...
          >
            <div className="aspect-[3/4] overflow-hidden mb-6 bg-champagne/10 relative">
              ...
              {/* Quick Add Overlay */}
              <div className={`absolute inset-0 bg-shadow/20 backdrop-blur-[2px] transition-opacity duration-500 flex items-center justify-center ${hoveredProduct === product.id ? 'opacity-100' : 'opacity-0'}`}>
                <Button 
                  onClick={(e) => handleAddToCart(e, product)}
                  className="bg-white text-black hover:bg-gold hover:text-white border-none min-w-[140px]"
                >
                  Quick Add
                </Button>
              </div>
            ...
            <div className="text-center">
               <h3 className="font-serif text-2xl text-shadow dark:text-smoke mb-1">{product.name}</h3>
               <p className="text-xs text-shadow/50 dark:text-smoke/50 uppercase tracking-widest mb-2">{product.category}</p>
